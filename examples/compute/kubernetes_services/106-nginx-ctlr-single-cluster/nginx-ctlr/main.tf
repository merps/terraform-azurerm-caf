...
provider "azurerm" {
  ...
  subscription_id = var.az_arm.subscription_id
  client_id       = var.az_arm.client_id
  client_secret   = var.az_arm.client_secret
  tenant_id       = var.az_arm.tenant_id
...
data "azurerm_virtual_network" "vnet" {
  name                = var.vnets.virtual_network_name
  resource_group_name = data.azurerm_resource_group.resourceGroup.name
}

data "azurerm_subnet" "subnet" {
  name                 = var.vnets.subnet_name
...
resource "azurerm_linux_virtual_machine" "controller" {
  ...
  source_image_reference {
    ...
    offer     = var.image.image_offer
    sku       = var.image.image_sku
    version   = var.image.image_version
  }
